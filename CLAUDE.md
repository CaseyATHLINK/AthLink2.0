...
- migrations/0008_host_logos.sql — APPLIED 2026-07-02, SUPERSEDED by 0011 recolor logo — see below. Added the hosts.logo_url column (still used). The original paused data-URL UI was never resumed; 0011 replaces that approach with a real storage bucket + a navy-recolor-at-upload treatment. NOTE: some host rows still hold the OLD full-colour data-URL in logo_url from this era — they now render un-recolored until re-uploaded through the new uploader.
...
- migrations/0011_host_logos_bucket.sql — APPLIED 2026-07-07 — public `host-logos` storage bucket (5MB, PNG/webp) + public-read/authenticated-write policies mirroring athlete-media. Backs the host/association self-logo feature: HostEditModal uploads a logo which uploadHostLogo/recolorLogoToNavy transforms ONCE, at save time, into a navy (--navy2 #1f4e80) monochrome watermark on transparent (luminance→opacity; near-white dropped), stores the PNG here, and writes its public URL to hosts.logo_url. Reuses the existing hosts.logo_url column (from 0008) — no new column.
- migrations/0099_cleanup_duplicate_policies.sql — OPTIONAL dedupe of redundant RLS policies

### Host logos (recolor-at-upload)
A host — federation, club, OR class association (an association is just a class-locked host, so its logo IS its "class logo"; no separate subsystem) — can upload a logo in the Edit page. Every upload is recolored ONCE at save time to a navy monochrome watermark on transparent (never a render-time CSS filter), stored in `host-logos`, and its URL saved to `hosts.logo_url`. The single recolored asset renders in exactly two places: the host directory card (bottom-right, ~34px) and the portal header (far right of the title row, ~120px, aligned to the globe; globe stays left, title middle, logo + action buttons on the right). Synthetic `class:*` portals have no host row → no logo. Helpers: `recolorLogoToNavy` + `uploadHostLogo` (module scope near uploadAthleteMedia); UI in HostEditModal (onUploadLogo prop, gated to canManage); threaded through applyDbHosts + saveHost.
