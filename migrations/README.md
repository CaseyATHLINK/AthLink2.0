...
| 0016 | `0016_binder_kind.sql` | ✅ **Applied 2026-07-16** (via Supabase MCP, task-chip session) | Binder namespaces as a real column: `scout_binders.kind` ('athletes'\|'results', default 'athletes') + backfill that moved #125's `res::`-prefixed rows onto the column and stripped the prefix from stored names. data/scout.js reads+writes the column with a prefix-reading fallback for stragglers. ⚠ The app-side switch shipped LATER than the migration (that session stalled uncommitted) — between 2026-07-16 and the scout-v3 PR, prod's prefix-reading code mis-grouped results folders under Athletes. |
| 0017 | `0017_claim_role_gating.sql` | ✅ **Applied 2026-07-16** (via Supabase MCP) | Athlete-claim role gating: only ATHLETE accounts (`profiles.role='athlete'`) may INSERT into `athlete_claims`, and an account can hold at most ONE live (pending/approved) claim ever. Adds security-definer helpers `is_athlete_account()` + `has_live_athlete_claim()`, re-creates `ac_insert_self` as `user_id=auth.uid() AND is_athlete_account() AND NOT has_live_athlete_claim()` (deliberately NO `is_athlink_admin()` branch — the placeholder would neuter it), adds partial unique index `uniq_live_claim_per_user` (user_id where status<>'denied', tightening 0003's approved-only guards), and backfill-deletes live claims filed by non-athlete accounts (1 scout-filed row removed). Verified with live tokens: scout INSERT 403, athlete first 201, athlete second 403. NOTE: 0016 (`scout_binders.kind`) was applied to prod by a parallel session — its ledger row arrives with its own PR. |
