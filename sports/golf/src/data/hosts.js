...
export function mockResearchIdentity(name,kind,countryHint){
  ...
  return {ok:true,mode:"identity",found:true,
    official_name:nm||"Sample Golf Club",
    acronym:(nm||"SSC").split(/\s+/).map(w=>w[0]||"").join("").toUpperCase().slice(0,5),
    website:"https://example.org",country:c,
    classes:kind==="association"?["Men's"]:["Men's","Women's","Amateur"],
    blurb:`${nm||"The organisation"} organises golf competitions.`,
    competitions:[
      {name:`${c} National Championship 2025`,year:2025,class:"Men's",url:"https://example.org/nats-2025.pdf"},
      {name:`${c} Amateur Series 2024`,year:2024,class:"Amateur",url:"https://example.org/amateur-2024.htm"},
      {name:`${c} Winter Open 2024`,year:2024,class:"Women's",url:null},
    ],
...
export function mockResearchCompetitions(name,kind,countryHint,site){
  const c=(countryHint&&countryHint.length===3?countryHint:"HKG").toUpperCase();
  const cls=["Men's","Women's","Amateur","Senior"];
...
export function mockParse(row){
  ...
  return {ok:true,name:row?.name||"Competition",date:row?.year?`01/06/${row.year}`:"",multi:false,
    entries:[{helm:"Alpha Player",crew:"",sail:"101",nat:"HKG",pdf_rank:1,races:[1]},
             {helm:"Bravo Player",crew:"",sail:"102",nat:"RSA",pdf_rank:2,races:[2]},
             {helm:"Charlie Player",crew:"",sail:"103",nat:"GBR",pdf_rank:3,races:[3]}],
