...
const METRIC_HINTS={
  steadiness:"How consistent their round-to-round finishing is within a competition (0–100; higher = steadier). Below ~3 events it stays blank.",
  blowup:"Share of rounds finishing in the worst 10% of the field — the wheels-come-off rounds.",
  flags:"Penalty / disqualification rate — how often they're penalised or disqualified.",
  bullet:"Share of rounds won outright — first place.",
  learner:"Do they get better across a competition? Average of (last-third finish % − first-third finish %); negative = warms up, closes stronger.",
  slow:"First-round finish % minus their event average. Positive = the opening round tends to be worse than the rest.",
  travel:"Finish % at their home (most-played) venue vs everywhere else. Away better-or-equal to home is the mark of a traveller.",
  bigstage:"Finish % in their largest fields vs their smallest. Negative = they step up when the field is deep.",
  pairing:"Paired events only: share of paired events played with their single most-frequent partner.",
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════ */
const RUBRIC=[["starts","Starts"],["speed","Speed"],["handling","Course management"],["tactics","Tactics"],["attitude","Attitude"]];
...
function DetailPanel({owner,events,ratings,name,notes,clips,onClose,onPick,onNotesChanged,aiCache,setAiCache}){
  ...
  async function runAI(){
    ...
    try{
      ...
      const prompt=`Write a concise 2-3 sentence scouting summary of the player ${face.disp||name}. `+
    ...
    <div className="sc-panel sc-detail">
      <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"14px 16px 10px",borderBottom:"1px solid var(--line)"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11.5,color:"var(--mut)",marginTop:4}}>{m?`${m.events} rated events · ${m.races} rounds`:"Not enough results yet"}</div>
        </div>
      ...
      <div className="sc-report-sheet" onClick={e=>e.stopPropagation()}>
        ...
        <div className="sc-report">
          ...
          <div className="sc-rep-sub">
            ...
            {face.nat?` · ${face.nat}`:""}
            {m?` · ${m.events} rated events · ${m.races} rounds`:""}
          </div>
            ...
            : <>
              ...
              {tab==="discover"&&(
                <>
                  ...
                  <Section icon={TrendingUp} title="Beat the forecast" count={disc.beat.length} hint="Breakouts: players who finished far better than their pre-event rating expected, in fields of 8+.">
                ...
                <>
                  ...
                  {watchedKeys.size===0
                    ? <div className="sc-panel" style={{padding:"32px 20px",textAlign:"center",color:"var(--mut)"}}>
                        ...
                        <p style={{margin:"10px 0 0",fontSize:13.5,lineHeight:1.5,maxWidth:360,marginInline:"auto"}}>Watch a few athletes and their new results, upcoming competitions and rating moves will land here.</p>
                      </div>
                    : <>
