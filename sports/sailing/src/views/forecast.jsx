  ...
  return(
    <div className="panel" style={{marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"14px 16px 4px"}}>
        ...
        <span style={{fontWeight:800,fontSize:14.5}}>Fleet forecast</span>
        <InfoHint text={FORECAST_HINT}/>
        <span style={{marginLeft:"auto",fontSize:11.5,color:"var(--mut)"}}>{sim.iters.toLocaleString()} simulated races · {N} boats</span>
      </div>
      <table>
        <thead><tr>
          <th title="Median simulated finish">Pred.</th>
          <th className="l">Boat</th>
          <th className="l">Sail #</th>
      ...
      {N>12&&(
        <div style={{padding:"8px 16px 12px"}}>
          <button className="btn ghost" style={{fontSize:12,padding:"5px 12px"}} onClick={()=>setShowAll(v=>!v)}>
            {showAll?"Show top 12":`Show all ${N} boats`}
          </button>
        </div>)}
