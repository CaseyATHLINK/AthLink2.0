...
function SailingGlobe({countryData,height=330,pulseIso=null}){
  ...
  const maxCount=React.useMemo(()=>Math.max(1,...Object.values(data)),[data]);
  const markerEntries=React.useMemo(()=>Object.keys(data)
    .filter(iso=>!isoSet.has(iso)&&GLOBE_CENT[iso])
  ...
  const redFor=(count)=>{
    ...
    return `rgb(${Math.round(238-70*t)},${Math.round(150-118*t)},${Math.round(140-110*t)})`;
  };
  ...
  React.useEffect(()=>{
    ...
    const size=()=>{
      const w=wrapRef.current.clientWidth,h=height;
    ...
    const project=(lon,lat,s)=>{
      const la=lat*D,lo=(lon+s.lon)*D,p0=s.lat*D,cl=Math.cos(la),sl=Math.sin(la);
    ...
    const runs=(ring,s)=>{
      ...
      for(const [lon,lat] of ring){const p=project(lon,lat,s);
        if(!p.vis){if(cur&&cur.length>1)out.push(cur);cur=null;continue;}
        if(!cur)cur=[];cur.push(p);}
    ...
    const draw=()=>{
      ...
      if(pulse!==s.lastPulse){
        s.lastPulse=pulse;
        if(pulse&&GLOBE_CENT[pulse]){const c=GLOBE_CENT[pulse];s.tlon=-c[0];s.tlat=Math.max(-45,Math.min(45,-c[1]*0.5));s.auto=false;}
      ...
      ctx.fillStyle='#0e2c50';ctx.fillRect(cx-R,cy-R,R*2,R*2);
      const shg=ctx.createRadialGradient(cx-R*0.3,cy-R*0.35,R*0.1,cx,cy,R);
      shg.addColorStop(0,'rgba(60,110,165,0.35)');shg.addColorStop(1,'rgba(8,28,52,0)');
      ...
      for(const c of GLOBE_COUNTRIES){
        ...
        ctx.fillStyle=competing?redFor(data[c.i]):'#d9e6f2';
        ...
        if(any)ctx.fill();
        ctx.strokeStyle=competing?'rgba(120,20,12,0.55)':'rgba(20,58,99,0.45)';ctx.lineWidth=competing?0.9:0.7;
      ...
      markerEntries.forEach(e=>{
        ...
        const r=4+5*Math.min(1,e.count/maxCount);
        ctx.fillStyle=redFor(e.count);ctx.beginPath();ctx.arc(p.x,p.y,r,0,7);ctx.fill();
      ...
      if(s.zoom>=1.6){
        ...
        for(const c of GLOBE_COUNTRIES){
          const cc=GLOBE_CENT[c.i];const nm=GLOBE_NAMES[c.i];if(!cc||!nm)continue;
          const p=project(cc[0],cc[1],s);if(!p.vis)continue;
          const competing=data[c.i]>0;
          ctx.lineWidth=2.4;ctx.strokeStyle=competing?'rgba(255,235,232,0.85)':'rgba(247,251,255,0.8)';ctx.strokeText(nm,p.x,p.y);
          ctx.fillStyle=competing?'#6e140c':'#163a63';ctx.fillText(nm,p.x,p.y);
        }
      }
      ctx.restore();

      // pulse spotlight (drawn on top, unclipped)
      if(pulse&&GLOBE_CENT[pulse]){
        const c=GLOBE_CENT[pulse];const p=project(c[0],c[1],s);
        if(p.vis){
          const ph=(performance.now()%1400)/1400;
          ctx.beginPath();ctx.arc(p.x,p.y,8+ph*26,0,7);ctx.strokeStyle=`rgba(230,60,50,${(1-ph)*0.9})`;ctx.lineWidth=2.5;ctx.stroke();
          ctx.beginPath();ctx.arc(p.x,p.y,5.5,0,7);ctx.fillStyle='#e63c32';ctx.fill();
    ...
    const move=ev=>{const s=stateRef.current;const q=pt(ev);
      ...
      let f=null;for(const e of markerEntries){if(e._sx==null)continue;if(Math.hypot(q.x-e._sx,q.y-e._sy)<=e._sr){f=e;break;}}
  ...
  },[data,markerEntries,maxCount,height]);

  const total=Object.values(data).reduce((a,b)=>a+b,0);
  const nCountries=Object.keys(data).length;
  return(
    <div ref={wrapRef} style={{position:'relative',width:'100%',borderRadius:14,overflow:'hidden',
         background:'radial-gradient(120% 120% at 30% 18%,#163a63 0%,#0a1f3a 72%)',border:'1px solid rgba(160,195,230,0.14)'}}>
      ...
      <div style={{position:'absolute',left:12,bottom:10,color:'#9fbdd9',fontSize:11,letterSpacing:0.3,pointerEvents:'none'}}>
        {nCountries} countr{nCountries!==1?'ies':'y'} · {total} regatta{total!==1?'s':''} · scroll to zoom · drag to spin</div>
    </div>
  );
}

/* ── FootprintModal: large globe + competitions list grouped by country ────── */
function FootprintModal({name,ag,countryCounts,onClose}){
  const [pulseIso,setPulseIso]=React.useState(null);
  // group history by country (alphabetical by country name)
  const groups=React.useMemo(()=>{
    const m={};
    ag.history.forEach(h=>{
      const ioc=h.ev.country||"";
      const iso=IOC_ISO[ioc]||"";
      const cname=GLOBE_NAMES[iso]||ioc||"Unknown";
      const key=iso||ioc||"ZZ";
  ...
  return(
    <div className="ov" onClick={onClose}>
      <div className="modal wide" onClick={e=>e.stopPropagation()} style={{maxWidth:980}}>
        <div className="mhead">
          <Flag size={18}/><h3>{name} — Competition footprint</h3>
          <button className="x" onClick={onClose}><X size={16}/></button>
        </div>
        <div style={{display:"flex",flexWrap:"wrap",gap:0,background:"var(--paper)"}}>
          <div style={{flex:"1 1 440px",minWidth:300,padding:18}}>
            <SailingGlobe countryData={countryCounts} height={460} pulseIso={pulseIso}/>
          </div>
          <div style={{flex:"1 1 360px",minWidth:280,maxHeight:496,overflowY:"auto",borderLeft:"1px solid var(--line)",padding:"8px 0"}}>
            {groups.map(g=>(
              <div key={g.cname}>
                <div style={{position:"sticky",top:0,background:"var(--sky)",color:"var(--navy)",fontWeight:700,
                     fontFamily:"'Barlow',sans-serif",fontSize:13,letterSpacing:.04+"em",padding:"7px 18px",
                  ...
                  <span style={{marginLeft:"auto",color:"var(--mut)",fontWeight:600}}>{g.items.length}</span>
                </div>
                {g.items.map((h,i)=>(
                  <div key={i}
                    onMouseEnter={()=>setPulseIso(g.iso||null)}
                    onMouseLeave={()=>setPulseIso(null)}
                    style={{padding:"11px 18px",borderBottom:"1px solid var(--line)",cursor:"default",transition:"background .12s"}}
                    onMouseOver={e=>e.currentTarget.style.background="#eef4fb"}
                    onMouseOut={e=>e.currentTarget.style.background="transparent"}>
                    <div style={{fontWeight:700,color:"var(--navy)",fontSize:14,marginBottom:3}}>{h.ev.name}</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:"4px 12px",fontSize:12.5,color:"var(--mut)"}}>
                      <span style={{color:h.row.rank<=3?"var(--gold)":"var(--ink)",fontWeight:700}}>
                        #{h.row.rank}<span style={{color:"var(--mut)",fontWeight:500}}> of {h.fleet} boats</span></span>
                      <span>{formatDate(h.ev.date)}</span>
                      {h.ev.class?<span style={{background:"var(--sky)",color:"var(--navy)",borderRadius:5,padding:"1px 7px",fontWeight:600,fontSize:11.5}}>{h.ev.class}</span>:null}
                    </div>
                  </div>
                ))}
              </div>
            ))}
            {groups.length===0&&<div style={{padding:24,color:"var(--mut)",fontSize:13}}>No competitions recorded yet.</div>}
          </div>
        </div>
      </div>
    </div>
