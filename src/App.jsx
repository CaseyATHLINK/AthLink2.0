...
/* ── SailingGlobe v2: self-contained canvas globe (no Mapbox, no API key) ────
   Props: countryData = { ISO2: count }  e.g. { HK:3, ES:1 }
   • Solid land with country borders
   • Competing countries filled red (deeper = more regattas)
   • Tiny territories (HK, SG) shown as red markers
   • Drag to spin · scroll/pinch to zoom · country labels appear when zoomed in */
...
function SailingGlobe({countryData}){
  ...
  const stateRef=React.useRef({lon:0,lat:-12,zoom:1,auto:true,drag:false,px:0,py:0,vlon:0.16,pinch:0});
  ...
  // ISO codes that have no fillable polygon -> draw as markers (e.g. HK, SG)
  ...
  // orient to the most-competed location on first paint
  React.useEffect(()=>{
  ...
  const redFor=(count)=>{
    // light salmon (1) -> deep red (max)
    const t=Math.min(1,count/maxCount);
    const r=Math.round(238-70*t);
    const g=Math.round(150-118*t);
    const b=Math.round(140-110*t);
    return `rgb(${r},${g},${b})`;
  };
  ...
  React.useEffect(()=>{
    ...
    const size=()=>{
      const w=wrapRef.current.clientWidth,h=330;
    ...
    // build visible runs of a ring (split where it goes behind the globe)
    const runs=(ring,s)=>{
    ...
    const draw=()=>{
      ...
      R=baseR*s.zoom;
      if(s.auto&&!s.drag&&s.zoom<1.15)s.lon+=s.vlon;
      ...
      // clip everything to the globe disc
      ctx.save();
      ctx.beginPath();ctx.arc(cx,cy,R,0,7);ctx.clip();

      // ocean (solid, with faint shading for sphericity)
      ctx.fillStyle='#0e2c50';ctx.fillRect(cx-R,cy-R,R*2,R*2);
      const sh=ctx.createRadialGradient(cx-R*0.3,cy-R*0.35,R*0.1,cx,cy,R);
      sh.addColorStop(0,'rgba(60,110,165,0.35)');sh.addColorStop(1,'rgba(8,28,52,0)');
      ...
      // countries: solid land fill (or red if competing) + border separators
      for(const c of GLOBE_COUNTRIES){
        const competing=data[c.i]>0;
        const fill=competing?redFor(data[c.i]):'#d9e6f2';
        // fill
        ctx.fillStyle=fill;
        ctx.beginPath();
        let any=false;
        for(const ring of c.r){
          for(const run of runs(ring,s)){
            any=true;
        ...
        if(any)ctx.fill();
        // border
        ctx.strokeStyle=competing?'rgba(120,20,12,0.55)':'rgba(20,58,99,0.45)';
      ...
      if(s.zoom>=1.6){
        ...
        for(const c of GLOBE_COUNTRIES){
          ...
          const competing=data[c.i]>0;
          ctx.lineWidth=2.4;ctx.strokeStyle=competing?'rgba(255,235,232,0.85)':'rgba(247,251,255,0.8)';
          ctx.strokeText(nm,p.x,p.y);
          ctx.fillStyle=competing?'#6e140c':'#163a63';
          ctx.fillText(nm,p.x,p.y);
        }
      }
    ...
    const move=ev=>{
      ...
      if(f){canvas.style.cursor='pointer';setTip({x:f._sx,y:f._sy,name:f.name,count:f.count});}
      else{canvas.style.cursor=s.drag?'grabbing':'grab';setTip(null);}
    };
    const up=()=>{const s=stateRef.current;s.drag=false;setTimeout(()=>{if(!s.drag&&s.zoom<1.15)s.auto=true;},2500);};
    ...
    const tend=ev=>{const s=stateRef.current;s.pinch=0;up();};
  ...
  },[data,markerEntries,maxCount]);

  const fillCount=Object.keys(data).filter(iso=>isoSet.has(iso)).length;
    ...
    <div ref={wrapRef} style={{position:'relative',width:'100%',borderRadius:14,overflow:'hidden',
      ...
      {tip&&(
        <div style={{position:'absolute',left:tip.x,top:tip.y-14,transform:'translate(-50%,-100%)',
             background:'rgba(8,24,45,0.95)',color:'#ffe9e6',padding:'6px 10px',borderRadius:8,
             fontSize:12,fontWeight:600,whiteSpace:'nowrap',pointerEvents:'none',
             border:'1px solid rgba(220,90,80,0.5)',boxShadow:'0 4px 14px rgba(0,0,0,0.4)'}}>
      ...
      <div style={{position:'absolute',left:12,bottom:10,color:'#9fbdd9',fontSize:11,letterSpacing:0.3,pointerEvents:'none'}}>
        {nCountries} countr{nCountries!==1?'ies':'y'} · {total} regatta{total!==1?'s':''} · scroll to zoom · drag to spin
      </div>
    </div>
  );
}
